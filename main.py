...
* ترجمه روان، طبیعی و حرفه‌ای باشد؛ ترجمه تحت‌اللفظی نباشد. طوری بنویس که انگار یک خبرنگار فارسی‌زبانِ فرمول‌یک، خودش این خبر را از صفر و به زبان مادری‌اش نوشته است، نه اینکه از انگلیسی ترجمه شده باشد. ساختار جمله‌ی انگلیسی (ترتیب فاعل-فعل-مفعول، جمله‌های وابسته‌ی پشت‌سرهم) را کپی نکن؛ جمله‌ها را کاملاً به سبک نگارش فارسی از نو بساز، حتی اگر یعنی ترتیب یا تعداد جمله‌ها با متن اصلی فرق کند.
* توجه حیاتی: این آزادی فقط در حد «جمله‌بندی و ساختار نوشتاری» است، نه محتوا. معنا، واقعیت‌ها، اعداد، نتایج، نام‌ها و نقل‌قول‌ها باید دقیقاً همان چیزی باشد که در متن منبع آمده — هیچ‌چیزی نباید کم، زیاد، یا تحریف شود. اگر مطمئن نیستی یک بازنویسی هنوز معنای دقیق منبع را حفظ کرده، بازنویسی ساده‌تر و نزدیک‌تر به منبع را انتخاب کن، نه بازنویسی خلاقانه‌تر را.
* بعد از نوشتن، متن را در ذهن خودت بازخوانی کن: اگر جمله‌ای هست که وقتی بلند می‌خوانی، غیرطبیعی یا شبیه ترجمه‌ی ماشینی به‌نظر می‌رسد، آن را کاملاً بازنویسی کن تا روان و طبیعی شود — اما فقط از نظر جمله‌بندی، نه از نظر معنا.
